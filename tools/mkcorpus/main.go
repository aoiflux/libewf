...
func runEwfacquire(outDir, rawDir, rawPath string, v variant) ([]string, digestInfo, error) {
	...
	args := []string{
		...
		"-l", logPath,
		"-C", "corpus", "-D", v.name, "-E", "mkcorpus",
		"-e", "libewf-test", "-N", "generated by mkcorpus",
	}
	if v.segmentBytes > 0 {
...
// readStoredDigests asks ewfinfo which digests the container actually holds.
// Using an independent implementation keeps the expectation non-circular: the
// value being asserted is not produced by the code under test.
